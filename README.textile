...
|id|AJAX results list id|'autocomplete'|
|hiddenField|if set to anything but @false@ it will create a hidden field that will hold the value of the selected option|'jsoncomplete-value'|
|data|extra data to send for the request. The plugin will always send @?value=$("input#id").val()@ in addition to these arguments|{}|
|afterSelect|Function to be performed after a click on a list element or a RETURN keypress|function(selectedElement){}|
